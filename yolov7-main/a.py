                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        if names[int(cls)] == "invoice":
                            ...
                            # 디버깅: ROI 화면에 표시
                            if view_img:
                                cv2.imshow("Invoice ROI", invoice_roi)
                                if cv2.waitKey(1) == ord('q'):
                                    return
                            else:
                                roi_path = save_dir / f"ROI_{time.time()}.jpg"
                                cv2.imwrite(str(roi_path), invoice_roi)
                            ...
                            if view_img:
                                ...
                                mask_path = save_dir / f"Mask_{time.time()}.jpg"
                            ...
                            for x, y, w, h in bounding_boxes:
                                ...
                                if view_img:
                                    ...
                                    roi_text_path = save_dir / f"Text_ROI_{time.time()}.jpg"
                                    cv2.imwrite(str(roi_text_path), roi)
                                    print(f"Text ROI saved at {roi_text_path}")
